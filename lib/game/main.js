...
.requires(
    ...
    MainGame = BaseScreen.extend({
        ...
        killGuy: function () {
            var retry = ig.game.spawnEntity(EntityRetry, this.screen.x - 400, 180);
            retry.tween({pos: {x: this.screen.x + 80, y: 180}}, 0.25).start();
            var quit = ig.game.spawnEntity(EntityQuit, this.screen.x + 400, 260);
