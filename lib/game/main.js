...
.requires(
    ...
    MainGame = BaseScreen.extend({
        ...
        killGuy: function () {
            retry = ig.game.spawnEntity(EntityRetry, this.screen.x - 400, 100);
            retry.tween({pos: {x: this.screen.x + 80, y: 100}}, 0.25).start()
            quit = ig.game.spawnEntity(EntityQuit, this.screen.x + 400, 180);
            quit.tween({pos: {x: this.screen.x + 95, y: 180}}, 0.25).start()
            this.guy.kill();
            delete this.guy;
        }
    });
